...
void Deletewithdate(struct Record* records, char num[10]) {
    int day, month, year;
    sscanf(num, "%d,%d,%d", &day, &month, &year);
    ...
    if (indexToDelete != -1) {
        for (int i = indexToDelete; i < 2; i++) {
            records[i] = records[i + 1];
        }
        // Decrease the size of the array by 1
        records[2] = { "", '\0', 0.0, 0, {0, 0, 0} };
    }
...
// 	SetConsoleOutputCP(1251);
	...
	records[0] = { "Papka", 'K', 4.75000, 4, {03,07,2022} };
	records[1] = { "Bubaga", 'K', 13.90000, 10, {03,04,2021} };
	records[2] = { "Calcurator", 'O', 411.00000, 1, {4,3,2022} };

	Draw(records);
	char a;
	char date[10];
	cout<< "Do want remove somethings with date? Y/n:"; cin>>a;
	if (a=='y' || a=='Y'){
		cout<<"enter Date(like this format dd,mm,yyyy like 03,04,2021 ):"; cin>>date;
		Deletewithdate(records, date);
